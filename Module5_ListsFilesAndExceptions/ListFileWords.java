...
/**
 * Prompt for a file to read. Read the file one line at a time,
 * break each line into individual tokens (words), and output
 * all tokens to the console, one token per output line.
 * 
 * @author mvail
 */
	...
	public static void main(String[] args) {
		...
		try {
			...
			while (fileScan.hasNextLine()) {
				...
				//create an additional Scanner to break the current
				// line into individual tokens (words) separated by
				// whitespace
				Scanner lineScan = new Scanner(line);
				
				//read each token from the line until we run out
				// of tokens
